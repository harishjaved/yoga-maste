...
const UpdateUser = () => {
    const {user} = useAuth();
    const userCredentials = useLoaderData(); // This will now contain data from the loader

    console.log(userCredentials); // Should log the user data
  ...
  <p className="text-center">
    Change details about <span className="font-bold text-red-400">{user?.displayName}</span>
  </p>
        ...
        <form className="space-y-4" onSubmit={handleFormSubmit}>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="text-[12px] ml-2 text-red-400">
                If the user is an instructor, then set skills; otherwise, leave it empty
              </p>
              <input
                className="w-full p-3 mt-2 text-sm border rounded-lg outline-none border-secondary"
                placeholder="Skills"
